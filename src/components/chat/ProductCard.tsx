...
export default function ProductCard({ card, cardType }: Props) {
  ...
  const hasDiscount = card.originalPrice && card.currentPrice && card.originalPrice > card.currentPrice;
    ...
    <div className="group flex-shrink-0 w-[240px] sm:w-[280px] snap-center rounded-2xl overflow-hidden bg-card ring-1 ring-foreground/10 hover:ring-foreground/20 shadow-sm hover:shadow-lg transition-all duration-300 hover-lift">
        ...
        {card.image ? (
          <img
            ...
            onError={(e) => {
              ...
              target.parentElement!.style.background = "linear-gradient(135deg, #222 0%, #333 50%, #222 100%)";
            }}
          />
        ) : null}
      ...
      <div className="p-3 space-y-2">
        ...
        {card.amenities && card.amenities.length > 0 ? (
          <div className="flex flex-wrap gap-1.5 pt-1">
            {card.amenities.slice(0, 4).map((a) => (
              <Badge key={a} variant="outline" className="text-[10px] px-2 py-0 rounded-full font-medium h-auto">
                {a}
              </Badge>
            ))}
        ...
        <div className="flex gap-2 pt-1">
          ...
          <Button size="sm" className="flex-1 rounded-xl text-[12px] font-semibold h-8" render={<a href={bookingUrl} target="_blank" rel="noopener noreferrer" />}>
            Book Now
          </Button>
        </div>
      </div>
